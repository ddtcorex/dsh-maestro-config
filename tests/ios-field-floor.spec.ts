...
// out to `inherit` for a compact scale — and every tap on those fields zoomed the page
// on iPhone (user report). Function beats pixels: this panel holds its fields at the
// same 16px on iOS. Android and desktop never carry the marker, so the compact scale
// they were designed with is untouched.
const source = readFileSync(
  resolve(dirname(fileURLToPath(import.meta.url)), '../src/client/MaestroSettings.tsx'),
  'utf8',
)
...
describe('Maestro settings panel holds the iOS 16px field floor', () => {
  ...
  it('holds the panel fields at 16px on iOS (opting out re-enables focus zoom)', () => {
    ...
    expect(block).toContain('html[data-mobile-nav-ios] [data-maestro-panel] input')
    expect(block).toContain('[data-maestro-panel] textarea')
    expect(block).toContain('[data-maestro-panel] select')
